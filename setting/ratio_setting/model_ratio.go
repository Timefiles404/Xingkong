...
func GetModelGroupRatio(name string, group string) (float64, bool) {
	if strings.TrimSpace(group) == "" {
		return -1, false
	}
	name = FormatMatchingModelName(name)
	groupRatioMap, ok := modelGroupRatioMap.Get(name)
	if !ok {
		return -1, false
	}
	ratio, ok := groupRatioMap[group]
	if !ok {
		return -1, false
	}
	return ratio, true
...
func GetModelGroupRatioMapCopy() map[string]map[string]float64 {
