...
func TestChannelAffinityHitPlaygroundResponsesByPromptCacheKey(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	affinityValue := fmt.Sprintf("pg-agent-%d", time.Now().UnixNano())
	cacheKeySuffix := buildChannelAffinityCacheKeySuffix(*codexRule, "gpt-5", "default", affinityValue)
	...
	rec := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(rec)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/pg/responses", strings.NewReader(fmt.Sprintf(`{"prompt_cache_key":"%s"}`, affinityValue)))
	ctx.Request.Header.Set("Content-Type", "application/json")
