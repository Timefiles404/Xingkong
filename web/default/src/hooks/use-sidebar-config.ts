...
const URL_TO_CONFIG_MAP: Record<string, { section: string; module: string }> = {
  ...
  '/dashboard/users': { section: 'data', module: 'detail' },
  '/usage-logs': { section: 'data', module: 'log' },
  '/usage-logs/common': { section: 'data', module: 'log' },
  ...
  '/profit-monitor': { section: 'admin', module: 'profit' },
...
function parseSidebarConfig(
  ...
  try {
    ...
    const merged = structuredClone(DEFAULT_SIDEBAR_MODULES)

    for (const [sectionKey, defaults] of Object.entries(DEFAULT_SIDEBAR_MODULES)) {
      const section = parsed?.[sectionKey]
      if (!section || typeof section !== 'object') continue

      for (const [moduleKey, defaultValue] of Object.entries(defaults)) {
        const candidate = section[moduleKey]
