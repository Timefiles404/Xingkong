...
export function WorkspaceSwitcher({
  ...
  const isDefaultWorkspace = activeWorkspace.id === WORKSPACE_IDS.DEFAULT
  const workspaceButtonContent = (
    <>
      ...
      <div className='grid flex-1 text-start text-sm leading-tight group-data-[collapsible=icon]:hidden'>
        <span className='truncate font-semibold'>{t('Overview')}</span>
        <span className='truncate text-xs'>
          {isDefaultWorkspace
            ? t('Open the dashboard overview')
      ...
      <SidebarMenuItem>
        {canSwitchWorkspace ? (
          ...
          <SidebarMenuButton
            asChild
            size='lg'
            className='rounded-xl border border-sidebar-border/60 bg-sidebar-accent/30 hover:bg-sidebar-accent/60 hover:text-sidebar-foreground active:bg-sidebar-accent/60 active:text-sidebar-foreground'
          >
            <Link to='/dashboard/overview'>{workspaceButtonContent}</Link>
          </SidebarMenuButton>
        )}
      </SidebarMenuItem>
    </SidebarMenu>
