...
const paymentSchema = z.object({
  ...
  ExternalRedemptionPurchase: z.string(),
  ExternalRedemptionPurchaseEnabled: z.boolean(),
  ExternalRedemptionPurchaseTitle: z.string(),
  ExternalRedemptionPurchaseDescription: z.string(),
  ExternalRedemptionPurchaseLink: z.string().refine((value) => {
    const trimmed = value.trim()
    if (!trimmed) return true
    return /^https?:\/\//.test(trimmed)
  }, 'Provide a valid URL starting with http:// or https://'),
...
export function PaymentSettingsSection({
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      ExternalRedemptionPurchaseEnabled: (() => {
        try {
          const parsed = JSON.parse(
            defaultValues.ExternalRedemptionPurchase || '{}'
          ) as {
            enabled?: boolean
          }
      ...
      ExternalRedemptionPurchaseTitle: (() => {
        try {
          const parsed = JSON.parse(
            defaultValues.ExternalRedemptionPurchase || '{}'
          ) as {
      ...
      ExternalRedemptionPurchaseDescription: (() => {
        try {
          const parsed = JSON.parse(
            defaultValues.ExternalRedemptionPurchase || '{}'
          ) as {
      ...
      ExternalRedemptionPurchaseLink: (() => {
        try {
          const parsed = JSON.parse(
            defaultValues.ExternalRedemptionPurchase || '{}'
          ) as {
  ...
  React.useEffect(() => {
    ...
    form.reset({
      ...
      ExternalRedemptionPurchaseEnabled: (() => {
        try {
          const parsed = JSON.parse(
            parsedDefaults.ExternalRedemptionPurchase || '{}'
          ) as {
            enabled?: boolean
          }
          return !!parsed.enabled
  ...
  const saveGeneralSettings = async () => {
    ...
    const sanitized = {
      ...
      ExternalRedemptionPurchase: JSON.stringify({
        enabled: values.ExternalRedemptionPurchaseEnabled,
        title: values.ExternalRedemptionPurchaseTitle.trim(),
        description: values.ExternalRedemptionPurchaseDescription.trim(),
        link: values.ExternalRedemptionPurchaseLink.trim(),
      }),
    }
    ...
    const initial = {
      ...
      ExternalRedemptionPurchase: JSON.stringify({
        enabled: (() => {
          try {
            const parsed = JSON.parse(
              initialRef.current.ExternalRedemptionPurchase || '{}'
            ) as { enabled?: boolean }
            return !!parsed.enabled
          } catch {
            return false
    ...
    if (
      normalizeJsonForComparison(sanitized.ExternalRedemptionPurchase) !==
      normalizeJsonForComparison(initial.ExternalRedemptionPurchase)
    ) {
      updates.push({
        key: 'payment_setting.external_redemption_purchase',
        value: sanitized.ExternalRedemptionPurchase,
      })
    }
            ...
            <div className='space-y-4 rounded-lg border p-4'>
              <div>
                <h4 className='text-base font-medium'>
                  {t('External redemption-code purchase')}
                </h4>
                <p className='text-muted-foreground text-sm'>
                  {t(
                    'Show a visible purchase entry in the user payment page and redirect users to an external platform to obtain redemption codes.'
                  )}
                </p>
              </div>
              ...
              <FormField
                control={form.control}
                name='ExternalRedemptionPurchaseEnabled'
                render={({ field }) => (
                  <FormItem className='flex flex-row items-center justify-between rounded-lg border p-4'>
                    <div className='space-y-0.5'>
                      <FormLabel className='text-base'>
                        {t('Enable external redemption-code payment')}
                      </FormLabel>
                      <FormDescription>
                        {t(
                          'When enabled, users will see a separate purchase card for obtaining redemption codes.'
                        )}
                      </FormDescription>
                    </div>
                    <FormControl>
              ...
              <div className='grid gap-4 md:grid-cols-2'>
                <FormField
                  control={form.control}
                  name='ExternalRedemptionPurchaseTitle'
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('Card Title')}</FormLabel>
                      <FormControl>
                        <Input
                          placeholder={t('e.g. Purchase redemption code')}
                          {...field}
                          onChange={(event) => field.onChange(event.target.value)}
                        />
                      </FormControl>
                ...
                <FormField
                  control={form.control}
                  name='ExternalRedemptionPurchaseLink'
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('Redirect Link')}</FormLabel>
                      <FormControl>
                        <Input
              ...
              <FormField
                control={form.control}
                name='ExternalRedemptionPurchaseDescription'
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        rows={3}
                        placeholder={t(
                          'e.g. Purchase a code on the external platform, then return here and redeem it below.'
                        )}
                        {...field}
                        onChange={(event) => field.onChange(event.target.value)}
                      />
                ...
                <FormItem>
                  ...
                  <FormControl>
                    {creemProductsVisualMode ? (
                      ...
                      <Textarea
                        rows={4}
                        placeholder='[{"name":"Basic","productId":"prod_xxx","price":10,"quota":10000,"currency":"USD"}]'
                        {...field}
                        onChange={(event) => field.onChange(event.target.value)}
                      />
                    )}
