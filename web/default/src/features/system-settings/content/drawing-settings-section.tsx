...
export function DrawingSettingsSection({
  ...
  return (
    <SettingsSection
      title={t('Drawing')}
      description={t(
        'Configure drawing availability and optional Midjourney behaviors.'
      )}
    >
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
          <FormField
            ...
            name='DrawingEnabled'
            render={({ field }) => (
              <FormItem className='flex flex-row items-start justify-between rounded-xl border p-4'>
                <div className='space-y-1 pe-4'>
                  <FormLabel className='flex items-center gap-2 text-base'>
                    <Sparkles className='size-4' />
                    {t('Enable drawing features')}
                  </FormLabel>
                  <FormDescription>
                    {t(
                      'Expose image-generation related capabilities to end users.'
                    )}
                  </FormDescription>
                </div>
                <FormControl>
              ...
              >
                <div>
                  ...
                  <div className='text-muted-foreground mt-1 text-sm'>
                    {t(
                      'Only needed when you actually use Midjourney callbacks or proxy-specific behaviors.'
                    )}
                  </div>
                </div>
