...
function hasAdvancedSettingsValues(values: ChannelFormValues): boolean {
  return Boolean(
    ...
    (values.profit_upstream_usd_per_cny || 0) > 0 ||
...
export function ChannelMutateDrawer({
  ...
  const { setOpen, open: dialogType } = useChannels()
  ...
  const isUpstreamRateConfigMode = dialogType === 'upstream-rate-config'
  ...
  }, [isEditing, channelData, form, isUpstreamRateConfigMode])
      ...
      <Sheet open={open} onOpenChange={handleOpenChange}>
        <SheetContent
          className={cn(
            ...
            isUpstreamRateConfigMode ? 'sm:max-w-xl' : 'sm:max-w-3xl'
          )}
        >
          <SheetHeader className='text-start'>
            ...
            <SheetDescription>
              {isUpstreamRateConfigMode
                ? t(
                    'Configure the upstream settlement exchange rate for this channel only. This value is used by profit accounting snapshots.'
                  )
                : isEditing
                ? t(
                ...
                <div className='bg-card space-y-4 rounded-xl border p-5'>
                  ...
                  <div className='grid gap-4 sm:grid-cols-2'>
                    ...
                    <div className='bg-muted/30 rounded-lg border p-3'>
                      ...
                      <div className='mt-1 font-medium'>{channelId ?? '-'}</div>
                    </div>
                  </div>

                  <FormField
                    control={form.control}
                    name='profit_upstream_usd_per_cny'
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          {t('Upstream Exchange Rate (1 CNY = ? USD)')}
                        </FormLabel>
                        <FormControl>
                        ...
                        <FormDescription>
                          {t(
                            "Used as this channel's upstream settlement exchange-rate snapshot default for later profit calculation. Example: 1 CNY = 0.1389 USD."
                          )}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
