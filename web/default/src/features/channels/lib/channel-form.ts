...
function normalizeUsdPerCnyRate(value: unknown): number {
  const raw =
    typeof value === 'number'
      ? value
      : typeof value === 'string'
...
export const channelFormSchema = z.object({
  ...
  profit_upstream_usd_per_cny: z.number().optional(),
...
export function transformChannelToFormDefaults(
  ...
  return {
    ...
    profit_upstream_usd_per_cny: profitUpstreamUSDPerCNY,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  if (
    typeof formData.profit_upstream_usd_per_cny === 'number' &&
    formData.profit_upstream_usd_per_cny > 0
  ) {
    settingsObj.profit_upstream_usd_per_cny =
      formData.profit_upstream_usd_per_cny
    if ('profit_upstream_cny_per_usd' in settingsObj) {
      delete settingsObj.profit_upstream_cny_per_usd
    }
  } else {
    if ('profit_upstream_usd_per_cny' in settingsObj) {
      delete settingsObj.profit_upstream_usd_per_cny
    }
    if ('profit_upstream_cny_per_usd' in settingsObj) {
      delete settingsObj.profit_upstream_cny_per_usd
    }
