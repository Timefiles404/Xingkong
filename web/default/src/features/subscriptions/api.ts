...
import type {
  ...
  SubscriptionWalletPurchaseResponse,
  SubscriptionCancelPreviewResponse,
...
export async function purchaseSubscriptionWithWallet(
  planId: number
): Promise<SubscriptionWalletPurchaseResponse> {
  const res = await api.post('/api/subscription/wallet/purchase', {
    plan_id: planId,
  })
  return res.data
}

export async function getSubscriptionCancelPreview(
  subId: number
): Promise<SubscriptionCancelPreviewResponse> {
  const res = await api.get(`/api/subscription/self/${subId}/cancel-preview`)
  return res.data
}

export async function cancelSubscriptionSelf(
  subId: number
