...
function GroupMultiplierNotesSection(props: {
  model: PricingModel
  groupRatio: Record<string, number>
}) {
  const { t } = useTranslation()
  const groups = props.model.enable_groups || []
  const modelGroupRatio = props.model.model_group_ratio || {}

  const visibleGroups = groups.filter((group) => modelGroupRatio[group] != null)
  if (visibleGroups.length === 0) return null

  return (
    <section className='border-b py-4'>
      <SectionTitle>{t('Model Group Multipliers')}</SectionTitle>
      <div className='space-y-1.5'>
        {visibleGroups.map((group) => {
          const systemRatio = props.groupRatio[group] || 1
          const customRatio = modelGroupRatio[group] || 1
          const finalRatio = systemRatio * customRatio
          return (
            <div key={group} className='flex items-baseline justify-between'>
              <span className='text-muted-foreground text-sm'>{group}</span>
              <span className='text-foreground font-mono text-sm tabular-nums'>
                {finalRatio.toFixed(2)}x
                <span className='text-muted-foreground ml-2 text-xs'>
                  {systemRatio.toFixed(2)}x × {customRatio.toFixed(2)}x
                </span>
              </span>
            </div>
          )
        ...
        <Table className='text-sm'>
          ...
          <TableBody>
            {availableGroups.map((group) => {
              const systemRatio = groupRatio[group] || 1
              const modelRatio = model.model_group_ratio?.[group] || 1
              const ratio = systemRatio * modelRatio
