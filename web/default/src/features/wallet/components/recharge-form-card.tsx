...
export function RechargeFormCard({
  ...
  const hasConfigurableTopup =
    ...
    enableWaffoTopup ||
    enableWaffoPancakeTopup
  const hasExternalRedemptionPurchase =
    !!externalRedemptionPurchase?.enabled &&
    !!externalRedemptionPurchase?.title &&
    !!externalRedemptionPurchase?.link
  ...
  return (
    <Card className='overflow-hidden'>
      ...
      <CardContent className='space-y-6 pt-6'>
        ...
        ) : hasExternalRedemptionPurchase ? (
          <div className='rounded-xl border border-dashed p-4'>
            <a
              href={externalRedemptionPurchase?.link}
              target='_blank'
              ...
              <div className='flex items-start justify-between gap-4'>
                <div>
                  ...
                  <div className='text-muted-foreground mt-1 text-sm'>
                    {externalRedemptionPurchase?.description ||
                      t(
                        'Get a redemption code from the external platform, then enter it below to add balance.'
                      )}
                  </div>
                </div>
        ...
        <div className='space-y-3 border-t pt-6'>
          ...
          {(topupLink || hasExternalRedemptionPurchase) && (
            <p className='text-muted-foreground text-xs'>
              {hasExternalRedemptionPurchase
                ? t(
                    'Get a code from the card above, then enter it here to add balance.'
                  )
                : t('Need a code?')}{' '}
              <a
                href={externalRedemptionPurchase?.link || topupLink}
                target='_blank'
                rel='noopener noreferrer'
                className='inline-flex items-center gap-1 underline-offset-4 hover:underline'
              >
                {hasExternalRedemptionPurchase
                  ? t('Open purchase page')
                  : t('Purchase here')}
