...
func migrateDBFast() error {
	...
	migrations := []struct {
		...
		{&SubscriptionUsageLedger{}, "SubscriptionUsageLedger"},
		{&WalletLot{}, "WalletLot"},
		{&SubscriptionLot{}, "SubscriptionLot"},
...
func migrateSubscriptionRollingLimits() {
	if DB == nil {
		return
	}
	...
	if DB.Migrator().HasColumn(&SubscriptionPlan{}, "quota_reset_period") {
		...
		if err := DB.Table("subscription_plans").
			Select("id, total_amount, quota_reset_period, quota_reset_custom_seconds, five_hour_amount, daily_amount, weekly_amount").
			...
			for _, plan := range plans {
				...
				case "daily":
					if plan.DailyAmount == 0 {
						updates["daily_amount"] = plan.TotalAmount
					}
				case "weekly":
					if plan.WeeklyAmount == 0 {
						updates["weekly_amount"] = plan.TotalAmount
					}
				case "custom":
					if plan.QuotaResetCustomSeconds == SubscriptionWindowFiveHours && plan.FiveHourAmount == 0 {
						updates["five_hour_amount"] = plan.TotalAmount
					}
				}
				if len(updates) == 0 {
					continue
				}
				_ = DB.Model(&SubscriptionPlan{}).Where("id = ?", plan.Id).Updates(updates).Error
			}
		}
	}
	...
	type legacySub struct {
		Id                  int
		PlanId              int
		AmountFiveHourLimit int64
		AmountDailyLimit    int64
		AmountWeeklyLimit   int64
	}
	var subs []legacySub
	if err := DB.Table("user_subscriptions").
		Select("id, plan_id, amount_five_hour_limit, amount_daily_limit, amount_weekly_limit").
		Where("amount_five_hour_limit = 0 AND amount_daily_limit = 0 AND amount_weekly_limit = 0").
	...
	for _, sub := range subs {
		...
		if err := DB.Select("five_hour_amount", "daily_amount", "weekly_amount").
			Where("id = ?", sub.PlanId).
			First(&plan).Error; err != nil {
			continue
		}
		if plan.FiveHourAmount == 0 && plan.DailyAmount == 0 && plan.WeeklyAmount == 0 {
			continue
		}
		_ = DB.Model(&UserSubscription{}).Where("id = ?", sub.Id).Updates(map[string]interface{}{
			"amount_five_hour_limit": plan.FiveHourAmount,
			"amount_daily_limit":     plan.DailyAmount,
			"amount_weekly_limit":    plan.WeeklyAmount,
		}).Error
	}
}
