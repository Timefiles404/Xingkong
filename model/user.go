...
func (user *User) Insert(inviterId int) error {
	...
	if common.QuotaForNewUser > 0 {
		if err := DB.Transaction(func(tx *gorm.DB) error {
			return CreateWalletLotFromQuotaGrantTx(tx, user.Id, int64(common.QuotaForNewUser), ProfitLotSourceGift, "new_user_signup")
		}); err != nil {
			return err
		}
	}
...
func (user *User) InsertWithTx(tx *gorm.DB, inviterId int) error {
	...
	if common.QuotaForNewUser > 0 {
		if err := CreateWalletLotFromQuotaGrantTx(tx, user.Id, int64(common.QuotaForNewUser), ProfitLotSourceGift, "new_user_signup"); err != nil {
			return err
		}
	}
...
func IncreaseUserQuota(id int, quota int, db bool) (err error) {
	return IncreaseUserQuotaWithSource(id, quota, db, ProfitLotSourceGift, "direct_quota_credit")
}

func IncreaseUserQuotaWithSource(id int, quota int, db bool, sourceType string, sourceNote string) (err error) {
	if quota < 0 {
	...
	// Positive quota changes must always create wallet lots. Batch mode cannot
	// preserve accounting lineage, so force a direct transaction here.
	if quota > 0 {
		return DB.Transaction(func(tx *gorm.DB) error {
			if err := tx.Model(&User{}).Where("id = ?", id).Update("quota", gorm.Expr("quota + ?", quota)).Error; err != nil {
				return err
			}
			return CreateWalletLotFromQuotaGrantTx(tx, id, int64(quota), sourceType, sourceNote)
		})
	}
