...
func updatePricing() {
	...
	for model, groups := range modelGroupsMap {
		...
		if groupRatio, ok := ratio_setting.GetModelGroupRatioMapCopy()[ratio_setting.FormatMatchingModelName(model)]; ok && len(groupRatio) > 0 {
			pricing.ModelGroupRatio = groupRatio
		}
