...
func GetTopUpInfo(c *gin.Context) {
	...
	data := gin.H{
		...
		"external_redemption_purchase": operation_setting.GetPaymentSetting().ExternalRedemptionPurchase,
	}
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			if err := model.DB.Transaction(func(tx *gorm.DB) error {
				...
				if err := model.CreateWalletLotFromTopUpTx(tx, topUp, int64(quotaToAdd), model.ProfitLotSourceTopUp); err != nil {
					return err
				}
				return model.RecordPaymentFeeLedgerForTopUpTx(tx, topUp)
			}); err != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 更新用户额度或利润账失败 trade_no=%s user_id=%d client_ip=%s quota_to_add=%d error=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), quotaToAdd, err.Error()))
				return
			}
