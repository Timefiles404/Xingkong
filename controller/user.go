...
func GetUserModels(c *gin.Context) {
	...
	capability := strings.TrimSpace(strings.ToLower(c.Query("capability")))
	if capability == "text" || capability == "image-generation" {
		imageFlags, err := model.ResolveImageGenerationFlags(models)
		if err != nil {
			common.ApiError(c, err)
			return
		}
		...
		for _, modelName := range models {
			isImageModel := imageFlags[modelName]
			if capability == "image-generation" && isImageModel {
				filtered = append(filtered, modelName)
			}
			if capability == "text" && !isImageModel {
				filtered = append(filtered, modelName)
			}
		}
		models = filtered
...
func ManageUser(c *gin.Context) {
	...
	case "add_quota":
		...
		case "add":
			...
			if err := model.IncreaseUserQuotaWithSource(user.Id, req.Value, true, model.ProfitLotSourceGift, fmt.Sprintf("admin_add:%s", adminName)); err != nil {
				common.ApiError(c, err)
				return
			}
		...
		case "override":
			...
			if err := model.DB.Transaction(func(tx *gorm.DB) error {
				...
				return model.SyncWalletLotsToUserQuotaTx(tx, user.Id, int64(req.Value), fmt.Sprintf("admin_override:%s", adminName))
			}); err != nil {
				common.ApiError(c, err)
				return
			}
