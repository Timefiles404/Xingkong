...
type WalletSubscriptionPurchaseRequest struct {
	PlanId int `json:"plan_id"`
}

type SubscriptionCancelPreviewResponse struct {
...
func PurchaseSubscriptionWithWallet(c *gin.Context) {
	userId := c.GetInt("id")
	var req WalletSubscriptionPurchaseRequest
	if err := c.ShouldBindJSON(&req); err != nil || req.PlanId <= 0 {
		common.ApiErrorMsg(c, "参数错误")
		return
	}
	sub, err := model.PurchaseSubscriptionWithWallet(userId, req.PlanId)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func GetSubscriptionCancelPreview(c *gin.Context) {
	userId := c.GetInt("id")
	...
	refundQuota, err := model.GetSubscriptionCancelPreview(userId, subId)
	if err != nil {
		common.ApiError(c, err)
		return
	}
	common.ApiSuccess(c, SubscriptionCancelPreviewResponse{RefundQuota: refundQuota})
}
...
func CancelSubscriptionSelf(c *gin.Context) {
	...
	refundQuota, downgradeGroup, err := model.CancelSubscriptionByUser(userId, subId)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func AdminCreateSubscriptionPlan(c *gin.Context) {
	...
	if req.Plan.FiveHourAmount < 0 || req.Plan.DailyAmount < 0 || req.Plan.WeeklyAmount < 0 {
		common.ApiErrorMsg(c, "时段限额不能为负数")
		return
	}
...
func AdminDeleteSubscriptionPlan(c *gin.Context) {
	...
	if err := model.DeleteSubscriptionPlanIfUnused(id); err != nil {
		common.ApiError(c, err)
		return
	}
	model.InvalidateSubscriptionPlanCache(id)
